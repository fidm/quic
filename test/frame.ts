...
suite('QUIC Frame', function () {
  ...
  suite('ACK Frame', function () {
    suite('parsing', function () {
      ...
      it('parse with parseFrame', function () {
        let buf = bufferFromBytes([0b01000000, 0x1c, 0x8e, 0x0, 0x1c, 0x1, 0x1, 0x6b, 0x26, 0x3, 0x0])
        let ackFrame = parseFrame(buf, new PacketNumber(1)) as AckFrame
        ok(ackFrame.largestAcked === 0x1c)
        ok(ackFrame.lowestAcked === 0x1)
        ok(ackFrame.delayTime === 142)
        ok(ackFrame.hasMissingRanges() === false)
      ...
      it('a frame without a timestamp', function () {
        ...
        let ackFrame = AckFrame.fromBuffer(buf) as AckFrame
      ...
      it.skip('a frame where the largest acked is 0', function () {
        let buf = bufferFromBytes([0x40, 0x1, 0xff, 0xff, 0x0, 0x0])
        let ackFrame = AckFrame.fromBuffer(buf)
  ...
  suite('STOP_WAITING Frame', function () {
    it('new StopWaitingFrame', function () {
      let headerPacketNumber = PacketNumber.fromBuffer(bufferFromBytes([0xff, 0x1f]), 2)
      let leastUnackedPacketNumber = PacketNumber.fromBuffer(bufferFromBytes([0xff, 0x0f]), 2)
      ...
      let buf = toBuffer(stopWaitingFrame)
      ok(buf.equals(bufferFromBytes([
        0x06,
        0x00, 0x10
      ])))
  ...
  suite('CONGESTION_FEEDBACK Frame', function () {
    it('new CongestionFeedbackFrame', function () {
      let congestionFeedbackFrame = new CongestionFeedbackFrame()

      strictEqual(congestionFeedbackFrame.type, 32)
      let buf = toBuffer(congestionFeedbackFrame)
  ...
  suite('PADDING Frame', function () {
    it('new PaddingFrame', function () {
      ...
      let buf = toBuffer(paddingFrame)
      ok(buf.equals(bufferFromBytes([0x00])))
      ok(buf.equals(toBuffer(PaddingFrame.fromBuffer(buf))))
    })
  ...
  suite('RST_STREAM Frame', function () {
    it('new RstStreamFrame', function () {
      ...
      let offset = Offset.fromBuffer(
        bufferFromBytes([0x01, 0x2, 0x03, 0x04, 0x05, 0x06]), 6)
      let rstStreamFrame = new RstStreamFrame(streamID, offset, error)

      strictEqual(rstStreamFrame.type, 1)
      let buf = toBuffer(rstStreamFrame)
      ok(buf.equals(bufferFromBytes([
        0x01,
        0x01, 0x00, 0x00, 0x00,
      ...
      ok(buf.equals(toBuffer(RstStreamFrame.fromBuffer(buf))))
    })
  ...
  suite('CONNECTION_CLOSE frame', function () {
    it('new ConnectionCloseFrame with QuicError(0)', function () {
      ...
      let buf = toBuffer(connectionCloseFrame)
      ok(buf.equals(bufferFromBytes([
        0x02,
        0x00, 0x00, 0x00, 0x00,
        0x00, 0x00
      ])))
      ok(buf.equals(toBuffer(ConnectionCloseFrame.fromBuffer(buf))))
    })
