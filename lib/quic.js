...
const { parsePacket } = require('../lib/packet')
...
function serverOnMessage (msg, rinfo) {
  debug(`server message: ${msg.length} bytes`)
  ...
  try {
    ...
    debug(`error parsing packet for ${packet.connectionID} from ${JSON.stringify(rinfo)}: ${err.message}`)
...
class Client extends Session {
  ...
  async connect (port, address) {
    ...
    this[kState].remoteAddr = SocketAddress.fromObject(addr)

    this[kSocket] = createSocket(addr.family === '6' ? 'udp6' : 'udp4')
    this[kSocket]
      .on('error', (err) => this.emit('error', err))
...
function clientOnMessage (session, msg, rinfo) {
  debug(`client message: ${session.id}, ${msg.length} bytes`)
  // The packet size should not exceed protocol.MaxReceivePacketSize bytes
  // If it does, we only read a truncated packet, which will then end up undecryptable
  if (msg.length > MaxReceivePacketSize) {
    debug(`receive too large data: ${msg.length} bytes`)
