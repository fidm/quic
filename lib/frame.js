...
const { Visitor, readUFloat16 } = require('./util')
...
class CongestionFrame extends QuicFrame {
  ...
  static fromBuffer (buf) {
    if (!QuicFrame.isCongestionType(buf[0])) throw new QuicError('QUIC_INVALID_FRAME_DATA')
...
class AckFrame extends QuicFrame {
  ...
  static fromBuffer (buf) {
    let type = buf[0]
    if (!QuicFrame.isACKType(type)) throw new QuicError('QUIC_INVALID_ACK_DATA')
...
class StreamFrame extends QuicFrame { // STREAM Frame
  ...
  static fromBuffer (buf) {
    let type = buf[0]
    if (!QuicFrame.isStreamType(type)) throw new QuicError('QUIC_INVALID_STREAM_DATA')
