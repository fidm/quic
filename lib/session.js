...
const { PingFrame } = require('./internal/frame')
...
class Session extends EventEmitter {
  ...
  _handleStream (streamFrame) {
    let streamID = streamFrame.streamID.value
    let stream = this[kStreams].get(streamID)
    if (!stream) {
      stream = new QUICStream(streamFrame.streamID, this, {})
      this[kStreams].set(streamID, stream)
      this.emit('stream', stream)
    }
    // TODO: ACK and reorder by offset
    stream.push(streamFrame.data)
  }
